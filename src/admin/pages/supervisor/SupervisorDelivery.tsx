  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <Badge variant="secondary">{drivers.length} سائق توصيل</Badge>
      ...
      <div className="rounded-xl border border-border overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-10 text-muted-foreground">لا يوجد سائقو توصيل</TableCell>
              </TableRow>
            ) : filtered.map(d => (
              <TableRow key={d.id}>
