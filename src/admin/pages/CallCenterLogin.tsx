...
const CallCenterLogin = () => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!mounted) return;
        if (roles.some((r) => r === "agent" || r === "admin")) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!roles.some((r) => r === "agent" || r === "admin")) {
        await signOutWithTimeout();
        toast({ title: "غير مصرح", description: "ليس لديك صلاحية الدخول إلى مركز الاتصال", variant: "destructive" });
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4" dir="rtl">
      <div className="w-full max-w-sm space-y-8">
        ...
        <p className="text-xs text-center text-muted-foreground">الوصول مقتصر على موظفي مركز الاتصال والمسؤولين فقط</p>
      </div>
    </div>
  );
};
