    ...
    >
      <div className="relative aspect-video bg-black/50">
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between z-10">
          <button onClick={togglePlay} className="w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 flex items-center justify-center hover:bg-black/70 transition-all">
            {playing ? <Pause className="w-4 h-4 text-white" /> : <Play className="w-4 h-4 text-white" />}
          </button>
          <button onClick={toggleMute} className="w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 flex items-center justify-center hover:bg-black/70 transition-all">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0} className="text-center mb-14">
          ...
          <h2 className="text-3xl md:text-4xl font-bold font-display mb-3">
            <span className="text-gradient-primary">{dir === "rtl" ? "شوف كيفاش خدامة HN Driver" : "See HN Driver in Action"}</span>
          </h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            {dir === "rtl" ? "مقاطع قصيرة تعرّفك على خدماتنا — النقل، التوصيل، والمنصة الكاملة" : "Short clips showcasing our services — rides, delivery, and the full platform"}
          </p>
        </motion.div>
