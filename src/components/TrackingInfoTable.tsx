...
const OrderRow = ({ order, idx }: { order: TrackingOrderRow; idx: number }) => {
  ...
  const handleCancel = () => {
    if (order.onCancel) order.onCancel();
  };
    ...
    <TableRow key={order.id || idx}>
      ...
      <TableCell className="text-center px-2 py-2">
        ...
        {order.referenceLabel && (
          <span className="text-[8px] text-muted-foreground block">{order.referenceLabel}</span>
        )}
      ...
      <TableCell className="text-center px-1 py-2">
        {!confirmed ? (
          <Button
            ...
            className="h-7 px-2 text-[10px] rounded-lg bg-gradient-to-r from-emerald-500 to-green-500 text-white font-bold shadow-sm"
          >
            {order.updating ? (
      ...
      <Table>
        ...
        <TableBody>
          {allOrders.map((order, idx) => (
            <OrderRow key={order.id || idx} order={order} idx={idx} />
          ))}
        </TableBody>
      </Table>
    </div>
