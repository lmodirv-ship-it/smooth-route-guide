  ...
  return (
    <div className="min-h-screen gradient-dark" dir={dir}>
      <div className="glass-strong sticky top-0 z-50 px-4 py-3 flex items-center justify-between">
        <div />
        <h1 className="font-bold text-foreground text-lg">{t.driver.tripHistoryTitle}</h1>
        <button onClick={() => navigate(-1)}><ArrowRight className="w-6 h-6 text-muted-foreground" /></button>
      </div>

      <div className="flex gap-2 px-4 mt-4">
        {[{ id: "all", label: t.driver.allFilter }, { id: "completed", label: t.driver.completedLabel }, { id: "cancelled", label: t.driver.cancelledLabel }].map(f => (
        ...
        {filteredTrips.map((trip, i) => (
          <motion.div key={trip.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div className="flex justify-between mt-3 pt-3 border-t border-border">
              ...
              <span className="text-xs text-muted-foreground flex items-center gap-1">
                <Clock className="w-3 h-3" /> {new Date(trip.created_at).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}
              </span>
            </div>
          </motion.div>
        ))}
