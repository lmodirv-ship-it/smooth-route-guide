...
const DeliveryHistory = () => {
  ...
  return (
    <div className="min-h-screen delivery-bg px-5 pt-6 pb-10" dir={dir}>
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate("/delivery")} className="p-2 rounded-xl bg-secondary">
          <ArrowRight className="w-5 h-5 text-foreground" />
        </button>
        <h1 className="text-lg font-bold text-foreground">{t.delivery.historyTitle}</h1>
        <div className="w-9" />
      </div>

      {loading ? (
        <div className="text-center py-16 text-muted-foreground">{t.delivery.loadingText}</div>
      ) : orders.length === 0 ? (
        <div className="text-center py-16">
          <Package className="w-12 h-12 text-muted-foreground mx-auto mb-3" />
          <p className="text-muted-foreground">{t.delivery.noPreviousOrdersHistory}</p>
        </div>
      ) : (
