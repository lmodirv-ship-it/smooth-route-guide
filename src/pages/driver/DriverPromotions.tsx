  ...
  return (
    <div className="min-h-screen gradient-dark pb-6" dir={dir}>
      ...
      <div className="px-4 mt-4">
        ...
        {promotions.length === 0 && <p className="text-center text-muted-foreground py-8">{t.driver.noPromotions}</p>}
          ...
          {promotions.map((p, i) => (
            <motion.div key={p.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-center justify-between mt-2">
                <span className="text-sm text-foreground">{t.driver.discountLabel} {p.discount_percent}%</span>
                <span className="text-xs text-muted-foreground">{t.driver.usedLabel} {p.used_count || 0}/{p.max_uses || "∞"}</span>
              </div>
              {p.expires_at && (
                <div className="flex items-center gap-1 mt-2">
                  <Clock className="w-3 h-3 text-muted-foreground" />
                  <span className="text-xs text-muted-foreground">{t.driver.expiresLabel} {new Date(p.expires_at).toLocaleDateString()}</span>
                </div>
              )}
            </motion.div>
          ))}
